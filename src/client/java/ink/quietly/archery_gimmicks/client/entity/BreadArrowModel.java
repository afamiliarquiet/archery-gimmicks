...
public class BreadArrowModel extends ArrowModel {
	public BreadArrowModel(ModelPart root) {
		super(root);
	}
...
//		);
		CubeListBuilder cross = CubeListBuilder.create().texOffs(0, 0).addBox(-14.0F, -8.0F, 0.0F, 16.0F, 16.0F, 0.0F, CubeDeformation.NONE);
		root.addOrReplaceChild("cross_1", cross, PartPose.offsetAndRotation(0, 0, 4, (float) (Math.PI / 2), (float) (-1 * Math.PI  / 4), 0.0F));
		root.addOrReplaceChild("cross_2", cross, PartPose.offsetAndRotation(0, 4, 0, 0, 0, (float) (1 * Math.PI  / 4)));
